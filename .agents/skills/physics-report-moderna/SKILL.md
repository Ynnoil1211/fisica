...
- **Formato Doble Columna Total:** Todo el cuerpo del reporte, incluyendo Referencias y Apéndices (A, B, C, D), debe mantener el formato IEEE de dos columnas continuas — sin excepción, incluidas las derivaciones extensas del Apéndice C y D.
- **Ecuaciones en Apéndices (Sin `\begin{aligned}` ni saltos de línea):** Prohibido usar `\\` o entornos multilínea dentro de un mismo bloque. Separar cada paso o término largo en bloques individuales e independientes de LaTeX (`$$paso 1$$`, `$$paso 2$$`).
- **Centrado y Escala Tipográfica de Ecuaciones (todo el reporte):** Todas las ecuaciones del reporte completo — Introduction, Results, Appendix C y Appendix D por igual — deben quedar perfectamente centradas en su columna y escaladas a 8.5 pt (OMML en Word), para evitar desbordamientos de margen y símbolos sobredimensionados en cualquier sección, no solo en los apéndices.
